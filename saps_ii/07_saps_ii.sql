...
DROP MATERIALIZED VIEW IF EXISTS saps_ii.SAPSII CASCADE;
CREATE MATERIALIZED VIEW saps_ii.SAPSII as
...
(
  ...
  inner join mimiciv.chartevents ce
...
-- they include some non-comorbid ICD-9 codes (e.g. 20302, relapse of multiple myeloma)
  ...
  (
    ...
    , cast(icd_code as char(5)) as icd9_code
...
--       , round( ( cast(ie.intime as date) - cast(pat.dob as date) ) / 365.242 , 2 ) as age
	, (floor(date_part('day', ie.intime - make_timestamp(pat.anchor_year, 1, 1, 0, 0, 0))/365.0) + pat.anchor_age) as age
...
left join saps_ii.gcsfirstday gcs
  on ie.stay_id = gcs.stay_id
left join saps_ii.vitalsfirstday vital
  on ie.stay_id = vital.stay_id
left join saps_ii.uofirstday uo
  on ie.stay_id = uo.stay_id
left join saps_ii.labsfirstday labs
